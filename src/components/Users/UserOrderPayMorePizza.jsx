...
const UserOrderPayMorePizza = () => {
  ...
  const deleteManyPayPizzaOrder = async (id) => {
    localStorage.removeItem('message');
    localStorage.removeItem('message1');
    localStorage.removeItem('message2');
    try {
      let result = await PayManyPizzaServices.deleteManyPayPizzaOrder(id);
      if (result) {
        getPayManyPizzaOrderId();
      }
  ...
  const handlePayNow = (total, pizzaItems, ok, custom) => {

    navigate("/pay/now", { state: { total, pizzaItems, ok, custom: custom } });
                  ...
                  return (
                    <tr key={item._id}>
                      ...
                      <td>
                        ...
                        <button onClick={() => deleteManyPayPizzaOrder(item._id)} className="btn btn-warning">
                          Order Cancel
                        </button>
...
export default UserOrderPayMorePizza;