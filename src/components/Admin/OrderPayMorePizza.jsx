...
const OrderPayMorePizza = () => {
  ...
  const previousMessage=JSON.parse(localStorage.getItem('success'));
  ...
  const getCustomerName = async (id) => {
    try {
      let result = await fetch(`http://localhost:7000/get-user/${id}`);
            ...
            orderList.map((item, index) => {
              ...
              return (
                <tr key={item._id}>
                  ...
                  <td>
                    <button onClick={() => deleteManyPayPizzaOrder(item._id)}>Order Done</button>
                  </td>
...
export default OrderPayMorePizza;