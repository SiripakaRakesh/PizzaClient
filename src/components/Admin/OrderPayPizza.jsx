...
const OrderPayPizza = () => {
  ...
  const getCustomerName = async (id) => {
    try {
      let result = await fetch(`http://127.0.0.1:7000/get-user/${id}`);
  ...
  const deletePayPizzaOrder = async (id) => {
    ...
    console.log(id);
    try {
      let result = await PayPizzaService.deletePayPizzaOrder(id);
      if (result) {
        getPayOrder();
      }
    } catch (error) {
            ...
            orderList.map((item, index) => {
              ...
              return (
                <tr key={item._id}>
                  ...
                  <td>
                    <button onClick={() => deletePayPizzaOrder(item._id)}>
                      Order Done
                    </button>
                  </td>
